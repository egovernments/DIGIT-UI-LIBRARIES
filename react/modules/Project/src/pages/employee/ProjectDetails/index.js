...
const ProjectDetails = () => {
  ...
  const projectSession = Digit.Hooks.useSessionStorage(
    "NEW_PROJECT_CREATE",
    {}
  );
  ...
  const handleParentProjectSearch = (parentProjectNumber) => {
    history.push(
      `/${window.contextPath}/employee/project/project-details?tenantId=${searchParams?.Projects?.[0]?.tenantId}&projectNumber=${parentProjectNumber}`
    );
  };
  ...
  const handleActionBar = (option) => {
    if (option?.name === "CREATE_ESTIMATE") {
      history.push(
        `/${window.contextPath}/employee/estimate/create-detailed-estimate?tenantId=${searchParams?.Projects?.[0]?.tenantId}&projectNumber=${searchParams?.Projects?.[0]?.projectNumber}`
      );
    }
    if (option?.name === "VIEW_ESTIMATE") {
      history.push(
        `/${window.contextPath}/employee/estimate/estimate-details?tenantId=${searchParams?.Projects?.[0]?.tenantId}&estimateNumber=${estimates?.[0]?.estimateNumber}`
      );
    }
    if (option?.name === "MODIFY_PROJECT") {
      if (
        ...
        estimates?.[0]?.wfStatus !== "REJECTED"
      ) {
        setToast({
          show: true,
          label: t("COMMON_CANNOT_MODIFY_PROJECT_EST_CREATED"),
          error: true,
        });
      } else {
  ...
  const HandleDownloadPdf = () => {
    ...
    Digit.Utils.downloadEgovPDF(
      ...
      { projectId, tenantId: searchParams?.Projects?.[0]?.tenantId },
      `Project-${projectId}.pdf`
    );
  };

  const { data } = Digit.Hooks.works.useViewProjectDetails(
    t,
    tenantId,
    searchParams,
    filters,
  ...
  const {
    data: estimates,
    isError: isEstimateSearchError,
    isLoading: estimateLoading,
  } = Digit.Hooks.works.useSearchEstimate(tenantId, {
    limit: 1,
    offset: 0,
    projectId: data?.projectDetails?.searchedProject?.basicDetails?.uuid,
  });

  useEffect(() => {
    const projectModifierRoles = ["PROJECT_CREATOR"];
    isProjectModifier = projectModifierRoles?.some((role) =>
      loggedInUserRoles?.includes(role)
    );
  }, [loggedInUserRoles]);

  useEffect(() => {
    const estimateViewerAndCreatorRole = [
      "ESTIMATE_CREATOR",
      "ESTIMATE_VERIFIER",
      "TECHNICAL_SANCTIONER",
      "ESTIMATE_APPROVER",
    ];
    isEstimateViewerAndCreator = estimateViewerAndCreatorRole?.some((role) =>
      loggedInUserRoles?.includes(role)
    );
  }, [loggedInUserRoles]);
  ...
  const setUniqueActions = (objToSet) => {
    const set =
      actionsMenu.filter((row) => row.name === objToSet.name).length === 0;
    if (set) {
  ...
  useEffect(() => {
    ...
    if (isEstimateSearchError && isEstimateViewerAndCreator) {
      setToast({
        show: true,
        label: t("COMMON_ERROR_FETCHING_ESTIMATE_DETAILS"),
        error: true,
      });
  ...
  useEffect(() => {
    ...
    let filterdNavConfig = navConfigs.filter(
  ...
  useEffect(() => {
    if (
      !window.location.href.includes("create-project") &&
      sessionFormData &&
