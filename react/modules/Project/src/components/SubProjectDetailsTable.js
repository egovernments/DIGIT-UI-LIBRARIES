...
const SubProjectDetailsTable = ({
  ...
  const renderSubProjectsFromSession = () => {
    ...
    for (
      let i = 1;
      i < sessionFormData?.withSubProject_project_subProjects?.length;
      i++
    ) {
      tableState.push({
        key: i,
  ...
  const [
    ...
    setSubProjectTypeOfProjectOptions,
  ] = useState([]);
  const [
    subProjectSubTypeOfWorkOptions,
    setSubProjectSubTypeOfProjectOptions,
  ] = useState([]);
  ...
  const [
    ...
    setSubProjectDetailsSelectedWard,
  ] = useState([]);
  const [
    subProjectDetailsLocalities,
    setSubProjectDetailsLocalities,
  ] = useState([]);
  const { data: cities } = Digit.Hooks.useTenants();
  const getCities = () =>
    cities?.filter((e) => e.code.includes(tenantId)) || [];
  ...
  useEffect(() => {
    let filteredSubTypeOfProject =
      subProjectTypeOfProjectOptions &&
      subProjectTypeOfProjectOptions?.projectSubType
        ? subProjectTypeOfProjectOptions?.projectSubType.map(
            (projectSubType) => ({
              code: projectSubType,
              name: `COMMON_MASTERS_${projectSubType}`,
            })
          )
        : [];
  ...
  useEffect(() => {
    setSubProjectDetailsLocalities(
      wardsAndLocalities?.localities[subProjectDetailsSelectedWard?.code]
        ? wardsAndLocalities?.localities[subProjectDetailsSelectedWard?.code]
        : []
    );
  }, [subProjectDetailsSelectedWard]);

  const getDropDownDataFromMDMS = (
    t,
    row,
    inputName,
    props,
      ...
      [{ name: options?.mdmsConfig?.masterName }],
      {
        select: (data) => {
          const optionsData = _.get(
            data,
            `${options?.mdmsConfig?.moduleName}.${options?.mdmsConfig?.masterName}`,
            []
          );
          return optionsData
            ...
            .map((opt) => ({
              ...opt,
              name: `${
                options?.mdmsConfig?.localePrefix
  ...
  const renderHeader = () => {
    return columns?.map((column, index) => {
      if (
        column.label === t("WORKS_ACTIONS") ||
        column.label === t("WORKS_SNO")
      ) {
        return (
          <th key={index} style={getStyles("SNO")}>
            {column?.label}
            {column?.isMandatory && "*"}{" "}
          </th>
        );
      ...
      if (column.label === t("WORKS_UPLOAD_DOCS")) {
        return (
          <th key={index} style={getStyles()}>
            {column?.label}
            {` (${t("WORKS_DOC_UPLOAD_HINT")})`}{" "}
          </th>
        );
      }
      return (
    ...
    switch (name) {
      case "projectName": {
        return (
          <>
            {errors &&
              (errors?.[formFieldName]?.[row.key]?.[name]?.type === "pattern" ||
                errors?.[formFieldName]?.[row.key]?.[name]?.type ===
                  "required") && (
                <CardLabelError
                  className={
                    !isErrorForDropdown
                      ? "projects-subProject-details-error"
                      : "projects-subProject-details-error dropdown-field"
                  }
                >
                  {t(`PROJECT_PATTERN_ERR_MSG_PROJECT_NAME`)}
                </CardLabelError>
              )}
          </>
        );
      ...
      case "endDate": {
        return (
          <>
            {errors &&
              (errors?.[formFieldName]?.[row.key]?.[`${name}_custom`]?.type ===
                "custom" ||
                errors?.[formFieldName]?.[row.key]?.[`${name}_custom`]?.type ===
                  "pattern" ||
                errors?.[formFieldName]?.[row.key]?.[`${name}_custom`]?.type ===
                ...
                >
                  {t(`COMMON_END_DATE_SHOULD_BE_GREATER_THAN_START_DATE`)}
                </CardLabelError>
              )}
          </>
        );
          ...
          <tr key={index} style={{ height: "50%" }}>
            ...
            <td style={getStyles("PROJECT_NAME")}>
              <div className="field" style={{ width: "100%" }}>
                <TextInput
                  ...
                  name={`${formFieldName}.${row.key}.projectName`}
                  inputRef={
                    selectedFormCategory === sectionFormCategory
                      ? register({
                          required: true,
                          pattern: /^[^\$\"<>?\\\\~`!@$%^()+={}\[\]*:;“”‘’]{1,50}$/i,
                          minLength: 2,
            ...
            <td style={getStyles()}>
              <div className="field" style={{ width: "100%" }}>
                <TextInput
                  style={{ marginBottom: "0px" }}
                  name={`${formFieldName}.${row.key}.estimatedCostInRs`}
            ...
            <td style={getStyles()}>
              <div
                className="field sub-projects-details-field-mt"
                style={{ width: "100%" }}
              >
                <Controller
                  control={control}
            ...
            <td style={getStyles()}>
              <div className="field" style={{ width: "100%" }}>
                <TextInput
                  style={{ marginBottom: "0px" }}
                  name={`${formFieldName}.${row.key}.geoLocation`}
                  inputRef={register({ required: false })}
                />
                    ...
                    return (
                      <MultiUploadWrapper
                        ...
                        allowedFileTypesRegex={
                          /(.*?)(pdf|docx|msword|openxmlformats-officedocument|wordprocessingml|document|spreadsheetml|sheet)$/i
                        }
            ...
            <td style={getStyles("SNO")}>
              {showDelete() && (
                <span onClick={() => removeRow(row)}>
                  <DeleteIcon fill={"#B1B4B6"} style={{ margin: "auto" }} />
                </span>
              )}
            </td>
          </tr>
      ...
      <table className="table reports-table sub-work-table project-details-table">
        ...
        <tbody>
          ...
          <tr>
            ...
            <td
              style={{ ...getStyles(2), textAlign: "center" }}
              onClick={addRow}
            >
              <span>
                <AddIcon
