...
function ApplicationDetailsWarningPopup({
  ...
  return (
    <React.Fragment>
      <Modal
        headerBarMain={
          <h1 className="heading-m">{t("PT_DUES_ARE_PENDING")}</h1>
        }
        headerBarEnd={
          <CloseBtn
            onClick={() => {
      ...
      >
        <Card>
          <div style={{ marginBottom: "30px" }}>
            <h1>
              {t("PT_YOU_HAVE")} ₹{action?.AmountDueForPay}{" "}
              {t("PT_DUE_WARNING_MSG2")}
            </h1>
          </div>
          <Row
            ...
            <ButtonSelector
              ...
              onSubmit={() =>
                window.location.assign(
                  `${window.location.origin}${action?.redirectionUrl?.pathname}`
                )
              }
              style={{ marginLeft: "10px" }}
            />
