...
function ApplicationDetailsToast({ t, showToast, closeToast, businessService }) {
  if (businessService?.includes("NewTL") || businessService?.includes("TL") || businessService?.includes("EDITRENEWAL")) {
    ...
    return <React.Fragment>{showToast && <Toast type={showToast.key} label={label} onClose={closeToast} />}</React.Fragment>;
    ...
    return <React.Fragment>{showToast && <Toast type={showToast.key} label={t(label)} onClose={closeToast} />}</React.Fragment>;
  }
}
