...
function ApplicationDetailsContent({
  ...
  const getTimelineCaptions = (checkpoint) => {
    if (
      ...
      return (
        <TLCaption data={caption} OpenImage={OpenImage} privacy={privacy} />
      );
    } else {
      const caption = {
  ...
  const getTextValue = (value) => {
    if (value?.skip) return value.value;
      ...
      return (
        <span style={{ color: "#0B6623" }}>{`${getTranslatedValues(
          value?.value,
          value?.isNotTranslated
        )}`}</span>
      );
    else if (value?.value === "Rejected")
      return <span style={{ color: "#FF0000" }}>{t(value?.value)}</span>;
    else
      return value?.value
    ...
    >
      <Card style={getCardStyles()} className={"employeeCard-override"}>
        ...
        {applicationDetails?.applicationDetails?.map((detail, index) => (
          ...
          >
            <React.Fragment key={index}>
                      ...
                      return (
                        <Row
                          ...
                          text={
                            value?.isImages ? (
                              <ViewImages
                                fileStoreIds={value?.fileStoreIds}
              ...
              {detail?.additionalDetails?.table
                ? detail?.additionalDetails?.table?.weekTable?.tableHeader && (
                    <>
                      ...
                      >
                        {t(
                          detail?.additionalDetails?.table?.weekTable
                            ?.tableHeader
                        )}
                      </CardSectionHeader>
                    </>
              ...
              {applicationDetails?.applicationData?.additionalDetails
                ?.fieldinspection_pending?.length > 0 &&
                detail?.additionalDetails?.fiReport && (
                  <InspectionReport
                    fiReport={
                      applicationDetails?.applicationData?.additionalDetails
            ...
            {doc.isNotDuplicate && <div> 
             ...
             <hr style={{color:"#cccccc",backgroundColor:"#cccccc",height:"2px",marginTop:"20px",marginBottom:"20px"}}/>
              ...
              {detail?.additionalDetails?.permissions &&
                workflowDetails?.data?.nextActions?.length > 0 && (
                  <PermissionCheck
                    ...
                    t={t}
                    permissions={detail?.additionalDetails?.permissions}
                  />
                )}
              ...
              {detail?.additionalDetails?.scruntinyDetails && (
                <ScruntinyDetails scrutinyDetails={detail?.additionalDetails} />
              )}
              ...
              {detail?.additionalDetails?.documentsWithUrl && (
                <DocumentsPreview
                  documents={detail?.additionalDetails?.documentsWithUrl}
                />
              )}
        ...
        {showTimeLine && workflowDetails?.data?.timeline?.length > 0 && (
          <React.Fragment>
            {workflowDetails?.breakLineRequired === undefined ? (
              <BreakLine />
              ...
              <Fragment>
                ...
                workflowDetails?.data?.timeline?.length === 1 ? (
                  <CheckPoint
                    isCompleted={true}
                    label={t(
                      `${timelineStatusPrefix}${workflowDetails?.data?.timeline[0]?.state}`
                    )}
                          ...
                          return (
                            <React.Fragment key={index}>
                              <CheckPoint
                                ...
                                label={t(
                                  `${timelineStatusPrefix}${
                                    checkpoint?.performedAction === "EDIT"
                                      ? `${checkpoint?.performedAction}_ACTION`
                                      : checkpoint?.[statusAttribute]
                                  }`
                                )}
