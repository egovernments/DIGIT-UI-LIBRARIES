...
const BoundaryFilter = (lowestLevel) => {
    ...
    const { data: hierarchyData1, refetch, isLoading } = Digit.Hooks.useCustomAPIHook(reqHierarchyData);
    ...
    const { isLoading: hierarchyLoading, data: hierarchy1 } = Digit.Hooks.useCustomAPIHook(reqCriteria);
    ...
    const boundaryOptionsUpdate = (boundaryType, values) => {
        ...
        // Extract only the selected objects, ignoring SyntheticBaseEvent
        const selectedValues = values.map((arg) => arg[1]); // Take only index 1 from each Arguments object
        ...
        setBoundaryOptions((prev) => {
            ...
            selectedValues.forEach((value) => {
                if (boundaryType !== lowestLevel) {
                    ...
                    // Convert existing entries to a Set to remove duplicates
                    ...
                    children.forEach((child) => {
                        if (!existingEntries.has(child.code)) {
                            updatedOptions[childBoundaryType][value?.code].push(child);
                            existingEntries.add(child.code); // Keep track of added entries
                        }
                    });
                }
            });
